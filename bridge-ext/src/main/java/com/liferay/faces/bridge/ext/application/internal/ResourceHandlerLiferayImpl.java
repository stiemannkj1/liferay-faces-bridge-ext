...
public class ResourceHandlerLiferayImpl extends ResourceHandlerWrapper {
	...
	private static final String CONFIGURATION_ADMIN_FQCN = ConfigurationAdmin.class.getName();
	...
	private final boolean projectStageProduction;
	...
	public ResourceHandlerLiferayImpl(ResourceHandler wrappedResourceHandler) {
		...
		this.projectStageProduction = ProjectStage.Production.equals(projectStage);
	}
	...
	private boolean isGlobalAMDLoaderExposed() {
		...
		if (projectStageProduction) {

			FacesContext facesContext = FacesContext.getCurrentInstance();
			ExternalContext externalContext = facesContext.getExternalContext();
			applicationMap = externalContext.getApplicationMap();
			amdLoaderExposedObject = applicationMap.get(GLOBAL_AMD_LOADER_EXPOSED_PROPERTY_NAME);
		}
		...
		if (amdLoaderExposedObject == null) {
			...
			ServiceReference configurationAdminReference = bundleContext.getServiceReference(CONFIGURATION_ADMIN_FQCN);
			...
			if (projectStageProduction) {
				applicationMap.put(GLOBAL_AMD_LOADER_EXPOSED_PROPERTY_NAME, amdLoaderExposed);
			}
		}
		else {
			amdLoaderExposed = (Boolean) amdLoaderExposedObject;
