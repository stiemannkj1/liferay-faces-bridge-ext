...
public class HeadResponseWriterLiferayImpl extends HeadResponseWriterLiferayCompatImpl {
	...
	public void endCDATA() throws IOException {
		...
		if (parentNode != null) {
			...
			writeNodeToHeadSection(currentNode, null, null);
			setCurrentNode(null);
		}
	}
	...
	@Override
	protected void writeNodeToHeadSection(Node node, String nodeName, UIComponent componentResource)
		throws IOException {
		...
		if (isElement(node)) {
			nodeAsString = XMLUtil.elementToString(node);
		}
		else {
		...
		logger.debug("Added resource to Liferay's <head>...</head> section, node=[{1}]", nodeName);
	}
	...
	private void addNodeToHeadSection(short nodeType, Object text) throws IOException {
		...
		if (currentNode != null) {
			...
			writeNodeToHeadSection(new NodeImpl(nodeType, text.toString(), null), null, null);
		}
	}
}
